...
async function initCirclePointsPrompt() {
    const promptSection = document.querySelector(".blackwood-circle-promo");

    if (!promptSection) {
        return;
    }

    const kicker = promptSection.querySelector(".blackwood-circle-promo-kicker");
    const title = promptSection.querySelector("#reader-record-circle-promo-title");
    const copy = promptSection.querySelector(".blackwood-circle-promo-content > p:not(.blackwood-circle-promo-kicker)");
    const signInButton = document.getElementById("reader-record-circle-signin");
    const continueButton = document.getElementById("reader-record-circle-continue");
    ...
    try {
        ...
        if (isSignedIn) {
            updateCirclePromptText({
                kicker,
                title,
                copy,
                kickerText: "Blackwood Circle Active",
                titleText: "Your Reader Record will earn +10 points",
                copyText: "You are signed in. Submit the form below and points will be added to your account after the record is filed."
            });

            hidePromptButton(signInButton);
            hidePromptButton(continueButton);
        ...
        updateCirclePromptText({
            ...
            title,
            copy,
            kickerText: "Blackwood Circle Points",
            titleText: "Earn +10 Circle points with a Reader Record",
            copyText: "Sign into The Blackwood Circle before submitting. Once your record is filed, your account will receive the points."
        });

        showPromptButton(signInButton);
        hidePromptButton(continueButton);
...
function updateCirclePromptText(options) {
    if (options.kicker) {
        options.kicker.textContent = options.kickerText;
    }

    if (options.title) {
        options.title.textContent = options.titleText;
    }

    if (options.copy) {
        options.copy.innerHTML = options.copyText.replace(
            "+10 Circle points",
...
function hidePromptButton(button) {
    if (!button) {
        return;
    }
...
function showPromptButton(button) {
    if (!button) {
        return;
    }
